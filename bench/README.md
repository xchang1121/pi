...
它使用同一内核，而不是另建文件系统或复制 grep 格式化逻辑；仅在受监督的资格子进程中绑定
Pi 0.84.1 的两个私有 imports。Actor 与投机共同使用虚拟时间、随机种子及封闭环境，默认原生
Actor 不变。该实验经过现有 resource world、gateway 和 EffectTransaction，检查完成采纳、
三个不同查询的输入重算，以及采纳前/搜索与上下文回读之间的宿主变化拒绝。过期后 Actor 执行一次。
原生 Actor 与共同配置的暖执行基线均取三次中位数，准备、投机和采纳成本分开报告；一次命中
耗时不是普遍加速保证。工作进程通信、通用取消和完整配额尚未通过资格，不据此启用生产工具。
