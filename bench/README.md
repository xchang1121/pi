...
19/21 个 PatternAware 批次的成员和因果边界逐项相同。核对成员时只排序副本；学习验证保留每次原始到达顺序，实际调用 Store 的 observeTurn/observeBatch/predict，仅将 durationMs 固定为 1。后续所有前沿与最终状态一致，分别得到 13/12 个模式和 38/47 个累计前沿预测。全部来源开启时，PatternAware 没有贡献额外 Actor 采纳；最终样本还保留了 Windows/WSL 的 7/24 条失败候选事件。单独开启 PatternAware 的完整复核为 10.692/36.323 s，仍与基线的完整 Actor 轨迹一致，两端各有两次 `resource.inputs` 读结果投影复用，精确预测匹配仍为零。其完整耗时高于单次关闭对照的 10.475/35.165 s；这些开关样本用于检查来源贡献，不证明自然预测或费用改善。
