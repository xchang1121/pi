...
- 默认主机按完整 K(a) 记忆封存输入上的求值结果，共用候选预算和回收；每次采纳在计算后、提交前验证一次新鲜度。移除默认 read 裁剪、范围扩张和未完成参数特例；大文件输出裁剪只保留显式适配器。
...
- 本轮完整测试为 Windows 532 通过、16 跳过；WSL 547 通过、1 跳过，共 548 项，两端 check/build/bench:check 通过。相对 `281c1dd`，TypeScript 生产源码减少 16 行，原生补丁生产代码增加 7 行；不新增依赖或源码文件。
