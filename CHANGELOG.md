...
- Windows/WSL 共用封存输入上的原版 read/ls，write/edit 沿用工作区事务。已有覆盖证明时先投影结果，投影明确拒绝后才尝试封存输入重算；异常与新鲜度失败不能绕过验证。
...
- `127589a` 完整测试为 Windows 530 通过、16 跳过；WSL 545 通过、1 跳过，共 546 项。两端 check/build/bench:check 通过；生产源码相对 `f3a1cb2` 减少 20 行，测试增加 59 行。
