...
`npm run setup:search` explicitly installs fixed, verified module bytes at `<agent-dir>/speculative-action/closed-search.wasm`; append `-- <file path>` for a qualification location. It installs no `rg` command, changes no PATH or Actor settings, and uses network only during explicit setup. Production worker ownership and the common-profile TUI are still pending: installation alone does not enable grep/find speculation.
