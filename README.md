...
资源按实际访问采集，共用 token 的预算、证据和所有权；原版 Pi 负责图片、截断和格式。封存后停止采集，新查询不能扩张权限；预算超限、链接逸出、特殊文件内容或语义变化均拒绝采纳。原生 Actor 观察另需证明包含祖先路径的完整窗口。Windows 的目录/junction 移走再移回可保持 inode 与时间戳，因此关闭原生观察；受控 `read/ls` 提前执行及跨轮次复用仍保留。图片选项和模型能力进入身份，路径/MIME helper 按 Pi 版本验收。原生 `grep/find` 仍须完整进程证明，包括配置、子进程和回读文件。
...
证书按“精确 exec 弱键 → 动态路径集 → 当前输入强键”查找。同路径集的多代证书共享一次当前依赖捕获，仍保留文件角色、metadata 策略、负查找父目录与私有条目排除。详见 [Bash 复用研究](./docs/bash-reuse-research.md)及 [WSL2 多历史基准](./bench/results/wsl2-pathset-2026-09-01.md)。

重放先装载并校验整个 CAS 输出/效果闭包，提交前准备 wire 与文件效果；之后删除底层 CAS 不会导致部分采纳后重跑。128 MiB 效果的真机结果见 [WSL2 artifact closure 基准](./bench/results/wsl2-artifacts-2026-09-01.md)。

嵌套进程 miss 共用外层工作区事务。同文件系统时钟栅栏后的 inode 变化令牌只筛选路径；不可变基线、精确前沿字节及稳定描述符读取才作内容权威。重叠、时钟不前进或 inode 语义不支持时禁止发布。驱动在首次变更时初始化，纯重放不付观察成本；移除整树快照后的 [WSL2 Bash 冷执行曾降低 7.4%](./bench/results/wsl2-workspace-frontier-2026-09-01.md)。
...
- K(a) 相等或显式证明请求覆盖才算预测命中；输入召回不把不同查询记成命中。新查询只在封存输入上求值，随后在提交前验证一次新鲜度。成功结果按完整 K(a) 随原候选有界保留，重复求值可跳过计算；所有结果共用原证据、字节预算和回收，新鲜度、权限、兼容性与提交检查不会跳过。
...
资源释放拒绝新读取，指纹失败排空已开始的读取与句柄。文件操作绑定在取消或错误后也等待原版内部操作及所属图像 worker 退出；并发调用各自拥有线程，不等待旁路 Actor 的线程。新目录按 Actor 当前 umask、默认 ACL 和继承策略创建，不复制私有目录权限；新建后回读或访问检查缺少权限证明时交还 Actor。会话关闭等待执行、封存、事务与清理结束，再回收工作区。

完整流式意图才触发准备，新建候选或转换前让出一次事件处理，正式调用已到达则跳过。预测与预览共用候选准入，预览各自持有兴趣；单方取消不撤销其他消费者的工作。正式采纳独立绑定执行器、授权、验新与提交，准备不发布权威结果或宿主效果。见[执行边界](./docs/bash-reuse-capability-lattice.md)。
...
PatternAware 在权威 batch 边界学习；多步模式对权威动作及真实输出做同轮续推，包括已采纳的 Drafter 结果。跨轮 `K(a)`/horizon 不变时沿用已有机会。入库事件与公开快照独立持有，续推历史不随后来的学习漂移；绑定缓存仅用于库内数据，每个对象最多 128 项，公开推导每次重新分析可变参数。
...
候选准入比较剩余等待、采纳和回退成本，已付成本不重复扣除。已完成候选至少有 4 个同动作、同采纳路径样本才按收益否决；首次转换与缓存查询分别采样，稀疏或宽泛样本不否决，Actor 耗时未知不强制重跑。任务按采纳生产者与查询转换去重：缓存命中计一次，淘汰重算另计，旧任务和被拒准备不算本次权威工作。主加速比为同次运行的反事实串行总时长/完整端到端时长；反事实保留实际开销，仅消除权威计算的重叠，故无重叠为 1×，有重叠大于 1×。独立开关对照另验隔离、预测、争用与回收的额外成本；开启更慢的条件须保留并继续减负，不能代入主分子。1×不表示没有额外成本。
