...
资源按实际访问采集，共用 token 的预算、证据和所有权。释放立即拒绝新读取，等待已开始的读取与句柄关闭；原版 Pi 负责图片、截断和格式。封存后停止采集，新查询不能扩张权限；预算超限、链接逸出、特殊文件内容或语义变化均拒绝采纳。原生 Actor 观察另需证明包含祖先路径的完整窗口。Windows 的目录/junction 移走再移回可保持 inode 与时间戳，因此关闭原生观察；受控 `read/ls` 提前执行及跨轮次复用仍保留。图片选项和模型能力进入身份，路径/MIME helper 按 Pi 版本验收。原生 `grep/find` 仍须完整进程证明，包括配置、子进程和回读文件。
...
学习库只接受当前持久化格式：事件去重存储，样本引用事件索引，且 gap 必须与所属样本池一致。读取不匹配格式时不迁移、不改写原文件；后续真实动作会重新积累并保存当前格式的学习结果。恢复的提示仍须经过当前 schema、K(a)、权限与执行证据验证，不直接授权采纳。
...
- runner 校验 Pi 0.84.1 文件执行器绑定，直接加载原版工具，传入绑定的图像缩放与模型图像能力；未知绑定或版本拒绝该路线，由 Router 选择其他合格后端或 Actor。投机同轮共享 BASE；共享观察首次采纳合并 `fs.verify` 与只读提交，后续逐次验新，写入保留 `fs.apply` 冲突检查。Actor 变更结算前使 BASE 失效。
