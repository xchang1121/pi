...
The resource route reuses Pi's own file-tool implementation, including image handling and output formatting. Its immutable input view belongs to the existing resource token, not another cache. Unknown reads, exceeded capture budgets, escaping links, unsupported content types, or an unstable capture window reject speculation. Read image options and the Actor model's image support are part of executor identity. Pi's private path/MIME helpers are version-qualified: other versions retain observation rather than receiving unverified pre-execution authority. Native `grep` / `find` still require a complete process provider; directory snapshots alone do not prove their configuration, subprocesses, or context-file reads.
