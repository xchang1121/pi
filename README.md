...
4. **调度与结算**：候选共用索引，Scheduler 比较原生执行及捕获/封存等待与采用成本；就绪结果持续亏损时回退，每四次决策探测恢复，同次候选枚举共用决策。`EffectTransaction` 验证提交。`host.execute` 通过 `prepareActorCall` 返回的执行专属句柄结算，`ActorAction` 独占证据；已选中结果不能回退。轮次只保留未结算调用，ID 与完成顺序不参与结果归属。
...
文件绑定只构造本次所需能力。释放时拒绝新读取，并排空已开始的操作、句柄及所属图像 worker，不等待旁路 Actor。新目录遵循 Actor 的 umask、默认 ACL 和继承策略；新建后观察缺少权限证明时回退。会话以原始标识持有轮次，创建、替换和关闭共用生命周期队列，不靠标识的字符串表示寻址。关闭等待执行、封存、事务和清理，再回收工作区。
