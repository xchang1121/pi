...
The existing resource/candidate store owns adoption and retained-input recomputation. Unproven inputs or budget limits (8 MiB input, 1 MiB result, 5-second execution deadline including startup) fail the selected find call; cancellation also waits for owned input operations to finish cleanup. Workers start on demand; disabling drains Actors, cancels producers and restores Native Pi. Obsolete search settings fall back to the default without setup errors or migration prompts. Windows and WSL are qualified; macOS still needs a real-host run. This fixed trusted executor is not an arbitrary Bash or hostile-JavaScript sandbox.
