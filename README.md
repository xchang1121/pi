...
资源按实际访问采集，共用 token 的预算、证据、所有权和释放；原版 Pi 负责图片、截断与格式。封存后停止采集，新查询不能扩张权限；预算超限、链接逸出、特殊文件内容或语义变化均拒绝采纳。原生 Actor 观察另需证明包含祖先路径的完整窗口。Windows 的目录/junction 移走再移回可保持 inode 与时间戳，因此关闭原生观察；受控 `read/ls` 提前执行及跨轮次复用仍保留。图片选项和模型能力进入身份，路径/MIME helper 按 Pi 版本验收。原生 `grep/find` 仍须完整进程证明，包括配置、子进程和回读文件。
