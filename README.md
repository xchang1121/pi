...
Run `npm run setup:search` from this package to install fixed, verified module bytes at `<agent-dir>/speculative-action/closed-search.wasm`; append `-- <file path>` for a qualification location. It installs no `rg` command, changes no PATH or Actor settings, and uses network only during explicit setup. Installation alone does not enable speculation.

For cross-platform `grep/find` pre-execution, select **Tools & execution → Execution routes → Search execution → Portable search**, then Apply. This explicitly selects the same captured-input executor for Actor and speculation (`searchExecution: "closed"`); **Native Pi** remains the default. It uses pinned search engines and workspace ignore rules, not ambient home/config, native rg/fd semantics, or command-text equivalence. Only retained inputs may be reused or recomputed through the existing resource/candidate store. Unproven inputs, budget limits (8 MiB input, 1 MiB result, 5-second call deadline including worker startup), or missing setup fail the selected call rather than silently switching semantics. Explicit refresh recovers repaired setup; workers start on demand. Disabling the plugin drains admitted Actor calls, cancels producers, closes workers and restores native Pi. Windows and WSL are qualified; macOS still needs a real-host run. This is not an arbitrary Bash or hostile-JavaScript sandbox.
