...
`unboundedExecutionGateEnabled` also defaults to `true`, but acts later and more narrowly: it partitions unbounded speculative execution by prediction source, tool, and concrete execution backend. Misses retain their measured execution cost; the first real Actor adoption cancels the useful candidate's cost and credits only realized `executionAheadMs`. After four ordinary negative samples—or two consecutive backend failures through the separate failure circuit—the prediction remains visible and losslessly matchable but its speculative process is paused. A matching Actor fallback contributes shadow benefit and bounded probes still execute, so a changed workload can reopen the partition. Actor previews, observation tools, workspace mutations, and authoritative tool results are never blocked by this gate. Set it to `false` to keep telemetry while admitting every otherwise-safe unbounded candidate.
