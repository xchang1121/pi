...
候选准入比较剩余等待、采纳和回退成本，已付成本不重复扣除。已完成候选至少有 4 个同动作、同采纳路径样本才按收益否决；首次转换与缓存查询分别采样，稀疏或宽泛样本不否决，Actor 耗时未知不强制重跑。任务按采纳生产者与查询转换去重：缓存命中计一次，淘汰重算另计，旧任务和被拒准备不算本次权威工作。主加速比为同次运行的反事实串行总时长/完整端到端时长；反事实保留实际开销，仅消除权威计算的重叠，故无重叠为 1×，有重叠大于 1×。独立开关对照用于检查隔离、预测、争用和回收的总成本差异，不能替代该分子或据此判定复用负收益。
