...
文件绑定只构造本次工具及所需能力。资源释放拒绝新读取；指纹失败、取消或错误仍排空已开始的操作、句柄和所属图像 worker，不等待旁路 Actor 线程。新目录遵循 Actor 当前 umask、默认 ACL 与继承策略；新建后观察缺少权限证明时回退。会话关闭等待执行、封存、事务和清理，再回收工作区。
