...
默认主机使用通用封存输入求值，不启用 `read` 范围裁剪、自动扩大读取范围或未完成参数的读取特例。新参数首次仍运行原计算，不声称能自动增量化任意黑盒程序。已有 `PI_READ_RANGE_PROJECTION_RULE` 仅供显式选择：它需配合真实输出覆盖证据，适用于大文件首次窄范围查询。元数据按实际消费字段取证；名字/类型查询不依赖文件大小，内容查询另行证明。Bash 依赖精确命令或原生进程证书，不能从 `| tail -n 20` 等文本推断等价。
