...
`setup:linux` installs the pinned `sandlock` CLI and, on x86-64/aarch64 hosts with `/dev/fuse`, an official hash-verified `fuse-overlayfs` static release under `~/.local`; it does not alter Pi or install a daemon. The runtime probes Landlock ABI 6+, unprivileged namespaces, bind mounts, Sandlock, strace, and the complete OverlayFS copy-up/whiteout/anonymous-transaction-clock/unmount lifecycle again on every host. A successful probe selects a host-visible copy-on-write workspace backed by one shared immutable Git baseline and a typed upper-layer journal. Its transaction clock is an unnamed `O_TMPFILE` in private upper-layer storage whose ordering against merged-view timestamps is proven by the probe, so Bash cannot observe a runtime control path. Any missing binary, unsupported marker, anonymous inode or clock projection, mount failure, or recovered lifecycle fault demotes later routes to the existing Git-worktree driver. An unresolved live mount and its pool are quarantined and retained rather than reclaimed, without blocking plugin shutdown. WSL must be version 2, and the checkout should live in its native Linux filesystem.
