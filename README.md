...
Open `/speculative-action` in the TUI. The menu names the two user-visible stores by purpose—live speculative results and reusable command history—and exposes both capacities plus reclaim/clear actions; clear requires confirmation. Every edit—including Enabled and Restore defaults—is staged until Apply; switching global/project scope reloads that layer, while a project file stores only differences from normalized global settings. Footer/status report how many Bash child commands were reused, how many came from earlier turns or stored history, and a matched-command time-saved estimate when the original process duration was recorded. That estimate compares prior child-process time with the complete nested reuse-request overhead and explicitly excludes shell, sandbox, model, and whole-Agent time; legacy history without timing remains `n/a`. Same-run overlap is still labelled as observed overlap. JSON byte limits use bytes; TUI memory inputs use MiB. Tool labels describe the available local fallback, and an unavailable isolation route always falls back to Actor execution.
