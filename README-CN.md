...
在 TUI 中打开 `/speculative-action`。菜单不再暴露 L1/L2 分层术语，而是按用途显示“实时投机结果”和“可复用命令历史”，并暴露两者容量及回收/清空动作；清空必须确认。包括 Enabled 和 Restore defaults 在内的修改都要到 Apply 才生效；切换全局/项目作用域会重新载入该层，而项目文件只保存相对规范化全局配置的差异。Footer/状态会直接显示复用了多少个 Bash 子命令、其中多少来自更早轮次或已有历史，以及匹配命令记录了原始进程耗时时基于该命令的估算省时。该估算用既有子进程耗时减去本次完整嵌套复用请求开销，并明确排除外层 shell、沙箱、模型和整个 Agent 的时间；没有耗时记录的旧历史显示 `n/a`。同次运行的重叠时间仍明确标为 observed overlap。JSON 容量单位是字节，TUI 内存输入单位是 MiB。工具标签会说明本地后备机制；不存在隔离路线时始终回退 Actor。
