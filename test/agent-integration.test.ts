...
describe("speculative action host", () => {
	...
	it("gates the whole Drafter batch by realized action execution ahead and recovers with a bounded probe", async () => {
		...
		const tool: AgentTool<typeof readSchema> = {
			...
			parameters: readSchema,
			execute: async (_id, input) => {
				if (input.path === "notes.txt") await new Promise((resolve) => setTimeout(resolve, 1_000));
	...
	it("pauses only negative-utility Bash execution and reopens it from Actor shadow benefit", async () => {
		const cwd = await temporaryWorkspace();
		const events: SpeculativeActionEvent<string>[] = [];
		let draftCommand = "echo wrong-1";
		let actorDelayMs = 0;
		let actorExecutions = 0;
		const tool: AgentTool<typeof bashSchema> = {
			name: "bash",
			...
			execute: async (_id, input) => {
				actorExecutions++;
				if (actorDelayMs > 0) await new Promise((resolve) => setTimeout(resolve, actorDelayMs));
				return { content: [{ type: "text", text: input.command }], details: {} };
			},
		};
		const sandboxFork = vi.fn(async (context: Parameters<SpeculativeAgentExecutionWorld["fork"]>[0]) => {
			await new Promise((resolve) => setTimeout(resolve, 10));
			const output = {
				result: {
					content: [{ type: "text" as const, text: String((context.args as { command?: unknown }).command) }],
					details: {},
				},
				isError: false,
			};
		...
		const host = createSpeculativeActionHost("session", {
			...
			getSettings: () => ({
				...
				tools: ["bash"],
				drafterMaxDepth: 0,
				drafterGateEnabled: false,
				unboundedExecutionGateEnabled: true,
			}),
			draftModel: model("draft"),
			complete: async () =>
				assistant(
					[{ type: "toolCall", id: "draft-bash", name: "bash", arguments: { command: draftCommand } }],
					"toolUse",
				),
			preflight: () => true,
			executionWorlds: [sandbox],
		...
		const actorFallback = async (turnID: string, command: string) => {
			const call = { turnID, id: `actor:${turnID}`, tool: "bash", args: { command }, tools: [tool] };
			expect(await host.consume(call)).toBeUndefined();
			const startedAt = performance.now();
			const result = await tool.execute(call.id, call.args, undefined);
			await host.actual({
				...call,
		...
		try {
			for (let index = 1; index <= 4; index++) {
				const turnID = `bash-gate-${index}`;
				draftCommand = `echo wrong-${index}`;
				await host.startTurn(startInput(tool, turnID));
				await waitFor(() =>
					events.some(
						(event) =>
				...
				await actorFallback(turnID, `echo actor-${index}`);
				await host.finishTurn(turnID);
			}

			// Each miss is re-armed once after the authoritative unbounded action invalidates its world.
			expect(sandboxFork).toHaveBeenCalledTimes(8);
			expect(host.unboundedExecutionGateSnapshot().entries[0]?.expectedNetBenefitMs).toBeLessThan(0);

			const blockedTurn = "bash-gate-5";
			draftCommand = "echo exact-5";
			await host.startTurn(startInput(tool, blockedTurn));
			await waitFor(() => host.runtime.inspect("session").executionBlockedPlanActions === 1);
			expect(sandboxFork).toHaveBeenCalledTimes(8);
			actorDelayMs = 250;
			await new Promise((resolve) => setTimeout(resolve, 250));
			await actorFallback(blockedTurn, draftCommand);
			await host.finishTurn(blockedTurn);

			expect(
				events.some(
					(event) =>
						event.type === "prediction" &&
						event.turnID === blockedTurn &&
						event.settlement.observation === "observed" &&
						event.settlement.match.matched &&
						event.settlement.match.adoption.status === "rejected" &&
						event.settlement.match.adoption.cause.stage === "admission" &&
						event.settlement.match.adoption.cause.code === "candidate_utility_suppressed",
				),
			).toBe(true);
			expect(host.unboundedExecutionGateSnapshot().shadowMatches).toBe(1);
			expect(host.unboundedExecutionGateSnapshot().entries[0]?.expectedNetBenefitMs).toBeGreaterThan(25);

			const reopenedTurn = "bash-gate-6";
			draftCommand = "echo exact-6";
			actorDelayMs = 0;
			await host.startTurn(startInput(tool, reopenedTurn));
			await waitFor(() =>
				events.some(
					(event) =>
						event.type === "candidate" &&
						event.turnID === reopenedTurn &&
						event.state.status === "succeeded",
				),
			);
			const hit = await host.consume({
				...
				id: "actor:reopened",
				tool: "bash",
				args: { command: draftCommand },
				tools: [tool],
			});
			expect(hit?.result.content).toEqual([{ type: "text", text: draftCommand }]);
			expect(sandboxFork).toHaveBeenCalledTimes(9);
			expect(actorExecutions).toBe(5);
			await host.finishTurn(reopenedTurn, true);
		} finally {
			await host.dispose();
		}
	}, 10_000);
