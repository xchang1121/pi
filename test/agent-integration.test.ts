...
describe("speculative action host", () => {
	...
	it("does not repeat world warm-up for an admitted sandbox candidate", async () => {
		const cwd = await temporaryWorkspace();
		const args = { command: "echo ready" };
		const output = {
			result: { content: [{ type: "text" as const, text: "ready" }], details: undefined },
			isError: false,
		};
		const prepare = vi.fn(async () => {});
		const fork = vi.fn<SpeculativeAgentSandbox["fork"]>(async (context) => {
			let state: "sealed" | "committed" = "sealed";
			return {
				output,
				backend: "test",
				resources: [],
		...
		const tool: AgentTool<typeof parameters> = {
			...
			parameters,
			execute: async () => {
				throw new Error("sandbox must execute the candidate");
			},
		};
		const host = createSpeculativeActionHost("session", {
			cwd,
			getSettings: () => ({ ...settings(), tools: { resourceCached: [], sandbox: ["bash"] } }),
			draftModel: model("draft"),
			complete: async () => assistant([{ type: "toolCall", id: "draft", name: "bash", arguments: args }], "toolUse"),
			resolveInvocation: () => ({
				executor: "test",
				process: {
					...args,
		...
		await host.startTurn(startInput(tool));
		await waitFor(() => fork.mock.calls.length === 1);
		expect(prepare).toHaveBeenCalledOnce();
		expect(await host.consume({ turnID: "turn-1", tool: "bash", args, tools: [tool] })).toEqual(output);
		await host.dispose();
	});
