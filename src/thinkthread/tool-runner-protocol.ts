...
export const THINKTHREAD_TOOL_RUNNER_PREFIX = "PI_SPECULATIVE_ACTION_RESULT_V1:";
export const THINKTHREAD_TOOL_RUNNER_MAX_REQUEST_BYTES = 1024 * 1024;

export type ThinkThreadToolName = "read" | "grep" | "find" | "ls" | "write" | "edit" | "bash";
...
export function decodeThinkThreadToolRunnerRequest(bytes: Uint8Array): ThinkThreadToolRunnerRequestV1 {
	...
	if (typeof record.tool !== "string" || !TOOL_NAMES.has(record.tool as ThinkThreadToolName)) {
	...
	if (record.shellPath !== undefined && typeof record.shellPath !== "string") {
		throw new Error("ThinkThread tool runner request shellPath is invalid");
	}
	if (record.shellCommandPrefix !== undefined && typeof record.shellCommandPrefix !== "string") {
		throw new Error("ThinkThread tool runner request shellCommandPrefix is invalid");
	}
	return {
		...
		...(typeof record.shellPath === "string" ? { shellPath: record.shellPath } : {}),
		...(typeof record.shellCommandPrefix === "string" ? { shellCommandPrefix: record.shellCommandPrefix } : {}),
	};
}
