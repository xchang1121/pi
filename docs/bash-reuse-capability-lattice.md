...
进程证书当前为 v7：规范身份采用精确 UTF-16 顺序，v6 弱键空间冷未命中，不能直接复用旧证书。
升级不执行清库，旧条目仍受原有 GC 策略管理；新证书预热后继续走原 L2、强验证和事务路径。
因此仅有 v6 旧证书、且没有合格生产者的机器，升级后不能据此获得复用收益。
...
预测匹配现在把已计算的 K(a) 关系随选中机会一起返回，不再对每个选中项调用第二次投影。
原 nearestPredictions 直接单次遍历选取每计划的最近机会，删除分组数组、过滤及排序；最近
已到期优先、否则最近未来、同期限首项及计划插入顺序均保留。工具名提示复用同一选择核心，
不计算投影也不领取预测。此关系只用于预测命中，不能代替候选授权、输出证据、收益或提交。
原无隔离夹具扩为 exact/future/due 三场景：真实 read.range 投影加计数，两个计划共 12 个
动作在旧实现的预览和正式 Actor 匹配中各调用 14 次，改后各 12 次。跨轮次保留、到期选择、
稳定同期限、投影距离和计划顺序断言通过；每个预测恰好结算一次，目标 Actor 一次 fallback
结算，预览不提前领取，无隔离时没有候选或投机执行。此调用计数不是原生耗时或收益测量。
两端 check/build、各 135 项定向及 55 文件单 worker 全量通过：Windows 534 passed / 16 skipped、
WSL 549 passed / 1 skipped；142 个 source/test 文件哈希一致。生产本步 −8 行，累计相对
485cdb2 为 32,698（−136）；测试本步 +24，累计 14,862（+631），没有新增测试文件。两端
顺序验证，无额外原生成本或压力扫描；ThinkThread、依赖和 CI 不变。完整目标继续保持未
结项，macOS/ARM64 与真实 ThinkThread Runtime 未验收，既有 Bash/grep 收益不是本次重测。

计划更新的所有权现在在入队前取得。原并发准入夹具以普通对象修改复现：同计划慢 K(a)
绑定阻塞后续修订时，producer 改写已经返回的 proposal/delta 的计划 ID、动作 ID 和路径，
旧实现会在等待结束后改用漂移的目标和输入。现在复用 PlanRuntime 原有动作校验与输入
捕获，在进入 planAdmissionTails 前拥有不可变头部、动作、依赖、删除项和输入；仅自己
签发的捕获记录可直接复用，原 producer 对象不被标记或冻结，也不复制其不透明 feedback。
应用时仍检查当前来源、修订序号和合并后的依赖图；proposal/delta 合并为同一应用路径，
删除旧的重复验证、组装及单用途转发函数。跨轮次锚点仍在实际应用时确定，既有异步调度、
权限与生命周期所有者不变。“仅下一步”在同轮设置下交接时过滤，不读取被排除的未来输入，
delta 仍可删除旧动作。原 PlanRuntime 夹具同时证明动作记录在捕获和应用之间保持同一实例、
输入只读取一次、原对象可重新提交校验，以及新鲜/已捕获记录的过滤行为；无新增测试用例
或文件。Runtime 夹具验证两种排队修订仍物化到原计划和原动作，并采纳原路径的执行结果。
两端 check/build、各 105 项定向及 55 文件单 worker 全量通过：Windows 534 passed / 16 skipped、
WSL 549 passed / 1 skipped；142 个 source/test 文件哈希一致。生产本步 −24 行，累计相对
485cdb2 为 32,674（−160）；测试本步 +31，累计 14,893（+662）。两端顺序验证，无额外压力
或原生成本扫描；ThinkThread、依赖和 CI 不变。完整目标未结项，macOS/ARM64 与真实
ThinkThread Runtime 仍未验收，既有 Bash/grep 收益记录不冒充本次测量。

父动作的语义替换现在沿依赖图更新执行身份。原父子隔离夹具扩为五场景：旧候选的复用验证
暂停期间，正常 observe delta 只替换同 ID 父动作、保持子动作及依赖 ID 不变；旧实现会把
旧父世界接到仍未完成绑定的新父动作下面，并产生错误的子 continuation。PlanRuntime 复用
原依赖校验遍历得到拓扑次序，让语义变化传播到依赖它的后继，沿既有退休、物化与启动路径
重建绑定；不增加 Runtime 回调层、队列或生命周期所有者。独立动作、仅预测元数据变化及
正常 Actor 采纳父动作仍保留执行。反序输入的三级依赖测试验证传播不依赖 producer 顺序，
并保留缺失依赖、自依赖、环路拒绝以及已领取旧预测的独立结算权。
同一夹具还复现：多个预测共享独占候选时，Actor 已领取其中一个并暂停验证，未领取的别名
随父动作退休会提前中止该事务，原可采纳结果变成 fallback。预测退休现在遵守既有候选
租约；Actor 仍可采纳一次，或取消后释放租约，由原结算路径回收。两种路径的旧子事务均在
完整关闭前清理一次，所有已输出预测结算身份唯一，完整关闭后清理次数等于实际执行次数。
夹具使用真实 EffectTransactionCoordinator 管理内存 WorldBranch 的提交和清理；checkpoint、
资源验证与工具输出仍为测试后端，1,000ms 只是提前启动的预测值，不是物理资源或成本资格。
两端 check/build、各 122 项定向及 55 文件单 worker 全量通过：Windows 539 passed / 16 skipped、
WSL 554 passed / 1 skipped；142 个 source/test 文件哈希一致。生产本步 −3 行，累计相对
485cdb2 为 32,671（−163）；复用原测试夹具，本步 +56 行，累计 14,949（+718），无新增测试
文件。两端顺序验证，无额外原生成本或压力扫描；ThinkThread、依赖和 CI 不变。完整目标
未结项，macOS/ARM64 与真实 ThinkThread Runtime 未验收，既有 Bash/grep 收益并非本次重测。

计划调度继续精简：原依赖校验已经得到拓扑顺序，现在将引用原节点的只读顺序保留在同一个
计划中，时序前向计算、关键路径反向传播。删除每次确认时的三套递归时序 memo、循环访问
集合、反向依赖索引和关键路径 memo；动作公开顺序仍由原节点 Map 保留，不增加执行所有者。
预测确认按原身份直接定位节点并核验机会实例，不再遍历所有计划；退休的旧机会仍可独立
结算，不能重算替换节点。修订重新取得拓扑顺序，预测元数据变化也重算耗时，不累积旧关键路径。
原图夹具扩为正序/反序输入，覆盖多父依赖、三级路径、不同期限、迟到 Actor 确认、耗时缩短、
删除后继和改接依赖；新增断言先在旧实现通过。小规模内存对照运行 32 组各 16 节点的图，
672 次公开状态快照和 426 次确认与旧实现一致；这不是性能测量，也不是原生进程资源证明。
两端 check/build、各 135 项定向及 55 文件单 worker 全量通过：Windows 540 passed / 16 skipped、
WSL 555 passed / 1 skipped；142 个 source/test 文件哈希一致。生产本步 −24 行，累计相对
485cdb2 为 32,647（−187）；测试本步 +14 行，累计 14,963（+732），无新增测试文件。两端
顺序验证，无额外原生成本或压力扫描；ThinkThread、依赖与 CI 不变。历史绝对代码预算仍
未达到，完整目标未结项；macOS/ARM64 与真实 ThinkThread Runtime 未验收，既有原生收益
记录不冒充本次测量。

依赖身份比较不再借助区域排序。新回归以不同的 Unicode ID（组合字符与预组合字符）复现：
仅重排相同依赖记录时，localeCompare 的相等排序结果会保留各自输入顺序，旧数组比较因而
错误退休已绑定 K(a) 和成功执行的子动作及其后继。现对已经捕获、字段已规范化的记录做
局部精确计数，不重新归一参数，也不排序或修改公开依赖数组；删除原排序辅助函数，不新增
状态缓存或生命周期所有者。条件和重复条目数量仍属于原比较语义，变化时继续更新执行身份。
从原图夹具合并出的 ASCII/Unicode 回归验证：相同依赖重排保留绑定、执行和后继身份，公开
数组保留新的输入顺序；默认条件与显式 execution_settled 相等，改变条件或重复数量则退休
相应依赖链。原始重复对象先各自捕获成独立只读记录，不能在比较时遗漏重复条目。该验证是
计划状态回归，不是原生执行耗时或资源证书资格测量。
两端 check/build、各 137 项定向及 55 文件单 worker 全量通过：Windows 542 passed / 16 skipped、
WSL 557 passed / 1 skipped；142 个 source/test 文件哈希一致。生产本步 −2 行，累计相对
485cdb2 为 32,645（−189）；测试本步 +17 行，累计 14,980（+749），无新增测试文件。两端
顺序验证，无额外原生成本或压力扫描；ThinkThread、依赖和 CI 不变。完整目标及历史绝对
代码预算仍未完成，macOS/ARM64 与真实 ThinkThread Runtime 尚未验收。

规范身份继续从区域排序中分离。四项旧实现失败回归证明：相同对象仅改变插入顺序会生成不同
JSON 和真实 K(a)，组合/预组合 Unicode 环境名会分裂进程弱键，依赖序列 [a,b,a] 与 [b,a]
不能归并为相同路径集。现在 JSON 命名键按 UTF-16 排序，整数索引仍按原数值顺序；环境名
也按精确字符串排序。动态依赖复用局部 Map，在单次归并中各计算一次精确身份，去重并排序
其键；删除反复计算身份的排序、字符串编码和相邻去重遍历。冲突证据仍拒绝，输入数组不被
改写，独立 Unicode 名称不被归一成同一个名称。目录实际枚举、事务日志和副作用阶段顺序未改。
证书升级到 v7 并只接受其规范语义 ID，删除同版本的旧计时型 ID 兼容分支。使用升级前真实
v6 哈希写入的持久化夹具证明旧弱键空间冷未命中、查询不修改旧字节，显式读取拒绝旧证书；
发布新证书后恢复原 L2 命中。仅把 epoch 改回 v6 的变异测试会继续命中旧空间，恢复 v7 后
通过隔离断言。不执行清库或削弱证据验证；旧缓存仍受原有 GC 管理，只有旧证书的消费机器需重新
获得兼容证书。v17/v18 模式库保留原提示 ID、Unicode 参数和反馈；提示仍需经过当前执行校验。
首次 Windows 全量的即时 GC 断言出现产物数量波动，未保留当时 mtime，不能唯一归因；
原零宽限夹具依赖文件时间与整数时钟的差值。现显式设置陈旧/未来时间，验证未来孤立产物
先保留、变陈旧后恰好回收一次；生产回收判断不变，没有靠重复压力运行掩盖失败。
最终两端 check/build 及 55 文件单 worker 全量通过：Windows 544 passed / 16 skipped、WSL
559 passed / 1 skipped。10 文件定向为 Windows 207 passed / 5 skipped（两批）、WSL 212 passed；
142 个 source/test 文件哈希一致。生产本步 −9 行，累计相对 485cdb2 为 32,636（−198）；测试
本步 +63，累计 15,043（+812），合并原重复契约夹具，无新增文件，净增两项参数化用例。
两平台顺序验证，无额外原生成本或压力扫描；ThinkThread、依赖和 CI 不变。完整目标与历史
绝对预算仍未完成，macOS/ARM64、真实 ThinkThread Runtime 与独立 Actor 证书生产尚未验收。

证书发布现在等待它已经启动的到期维护。原 GC 虽登记在存储内部 maintenance 队列中，put
却提前返回，使等待发布的既有 Gateway 也可能先完成关闭，随后仍有文件扫描或删除。现在
只把该 GC Promise 纳入原发布调用，不增加队列、关闭接口或生命周期所有者；GC 触发条件、
存储内部串行规则和未到期路径不变。到期维护时间属于此次调用的真实完成时间，维护失败仍不
替换已经完成的发布结果。进程 handoff 先暴露内存候选再持久化的顺序不变，原阻塞持久化
期间的接管回归通过；这不是对维护耗时或原生收益的新测量。
原真实存储/CAS 夹具加入目录读取屏障，并覆盖维护成功、失败两种情况。两种旧实现均在
GC 被阻塞时报告 Gateway 关闭完成，恢复等待后均通过：关闭前可读到已发布证书，但两次
关闭请求都必须等待维护结束；发布执行器始终只调用一次，IO 失败后仍保留 true 结果且可
继续回收。夹具使用合成证书和既有 Gateway 已准入调用，不证明独立 Actor 原生证书生产。
CAS 独立使用断言合入闭包夹具，保留后备文件消失时已借用字节仍可读取的验证。
两端 check/build、各 100 项定向和 55 文件单 worker 全量通过：Windows 544 passed / 16 skipped、
WSL 559 passed / 1 skipped；142 个 source/test 文件哈希一致。生产本步不增行，累计相对
485cdb2 仍为 32,636（−198）；测试本步 +24，累计 15,067（+836），总用例数不增、无新增文件。
两平台顺序验证，无额外原生成本或压力扫描；ThinkThread、依赖、CI 与证书 v7 语义未改。
完整目标及历史绝对预算仍未完成，macOS/ARM64 与真实 ThinkThread Runtime 未验收。

维护内部的失败并行分支也纳入完成边界。仅等待 GC 返回仍不充分：证书/产物目录扫描或
批量删除使用 Promise.all，某一项失败会先返回，其他已启动的 IO 仍可能存活。现在这两处
保留本批 Promise，失败后等待同批全部结束，再重新抛出最先收到的原始错误；成功路径
仍直接使用原并行结果。不增加队列、维护所有者、转发层或聚合错误，也不改回收选择策略。
原存储夹具变为 held / scan_failure / delete_failure 三场景：让一项失败，同时保持另一项
真实文件扫描或删除处于屏障内。旧实现的普通阻塞场景通过，而两种兄弟失败场景均已提前
关闭 Gateway；新实现均等待屏障释放。错误信号与物理 IO 结束分别控制，夹具自己的 finally
也等待挂起 IO，失败回归不遗留后台任务。发布结果仍为 true，执行器一次，原错误对象不变；
后续 GC 正常完成，已由兄弟删除的产物不重复计数。使用真实临时 CAS 与注入的 IO 故障，
不是原生 Bash、独立 Actor 证书生产或收益资格。
两端 check/build、各 101 项定向及 55 文件单 worker 全量通过：Windows 545 passed / 16 skipped、
WSL 560 passed / 1 skipped；142 个 source/test 文件哈希一致。生产本步 +8 行用于两处失败
收尾，累计相对 485cdb2 为 32,644（−190），仍不净增；测试本步 +13，累计 15,080（+849），
只扩展既有夹具、净增一项场景，无新增文件。两平台顺序验证，无额外原生成本或压力扫描；
ThinkThread、依赖与 CI 不变。完整目标和历史绝对预算仍未完成，macOS/ARM64 与真实
ThinkThread Runtime 未验收；既有原生收益记录不冒充本次测量。

不可变发布的临时文件也遵守实际所有权。原 writeFile 在清理区外完成排他创建和写入；真实
写入 3 字节后注入 EIO 的回归证明，调用虽拒绝，磁盘仍留有部分临时文件。现在先用原生
open(wx) 取得创建成功的文件句柄，写入成功或失败都等待句柄关闭，再链接发布；外层 finally
只清理已取得所有权的临时文件。排他创建失败尚未进入该清理区，不能删除其他发布者的文件；
EEXIST 去重仍仅适用于目标链接。复用原发布函数和调用生命周期，无新队列、转发层或依赖。
原独立 CAS 闭包夹具扩为部分写入失败、同名临时文件冲突、链接失败三场景；旧实现仅部分
写入场景失败。真实句柄关闭一次，已拥有的临时文件不存在，冲突文件的哨兵字节不变且未被
请求删除，失败均不发布目标。解除故障后原路径可重试，已验证闭包仍可在后备文件删除后
读取；原证书和产物去重断言保留。这是小规模真实文件 IO 故障测试，不是原生进程收益资格。
两端 check/build、各 103 项定向及 55 文件单 worker 全量通过：Windows 547 passed / 16 skipped、
WSL 562 passed / 1 skipped；142 个 source/test 文件哈希一致。生产本步 +3 行，累计相对
485cdb2 为 32,647（−187）；测试本步 +44，累计 15,124（+893），复用原夹具、净增两场景，
无新增文件。两平台顺序验证，无额外原生成本或压力扫描；ThinkThread、依赖和 CI 不变。
完整目标及历史绝对预算仍未完成，macOS/ARM64 与真实 ThinkThread Runtime 未验收。

进程证书封存不再通过公开键入口重复规范化内部记录。原型头部、环境字段、stdin 和描述符
先捕获再校验，依赖在复制和排除项数组规范化后校验；非数组排除项仍拒绝，不被 Set 隐式
转成合法数组。封存复用这些同一份记录直接计算弱键、强键，删除仅此处调用的依赖证书
组装辅助函数；公开键入口继续校验外部输入，持久化解析仍重建并核对 v7 身份。不增加
信任注册表、缓存、转发层或生命周期所有者，也不把构造器的冻结结果当作已校验证据。
既有 ASCII/Unicode 夹具记录真实 Object.freeze 调用：旧实现各复制两份原型，三条原始依赖
产生五份排除项数组；新实现分别为一份和三份，原始顺序与数据不被改写。两份重构前的 v7
身份固定为回归值，公开弱键/强键与封存结果一致；浅冻结的非法原型、缺失 stdin 和非法
排除项仍被拒绝。小规模内存对照中，24 组完整证书、键与持久化往返无差异，384 组非法
输入操作的旧/新拒绝结果一致；这些复制次数和对照不是原生进程耗时或资源资格证明。
两端 check/build、各 114 项定向及 55 文件单 worker 全量通过：Windows 547 passed / 16 skipped、
WSL 562 passed / 1 skipped；142 个 source/test 文件哈希一致。生产本步 −3 行，累计相对
485cdb2 为 32,644（−190）；测试本步 +36，累计 15,160（+929），复用原夹具、不增用例或文件。
两平台顺序验证，无额外原生成本或压力扫描；ThinkThread、依赖、CI 与 v6 冷未命中规则不变。
完整目标和历史绝对预算仍未完成，macOS/ARM64 与真实 ThinkThread Runtime 未验收。

进程查找现在接收已经绑定的静态身份。破坏性接口迁移：ProcessReuseRequest 的 prototype
改为必填 weakKey；调用方在捕获实际调用后用 processWeakKey(boundPrototype) 校验并生成
一次，不再把原型传给每次查询。原生整条 Actor/投机入口和 dispatcher/held-exec 子进程入口
均已更新；有交接和收益判断时也沿用同一键，原型仍保留给实际执行和证书封存。查找器
捕获并检查键格式，不提供可选的绕过参数；键本身不是执行许可、资源证据或回放资格。
生产者策略、live 同键条件、当前动态依赖强验证、产物闭包及观察合同继续按原路径检查。
同时删除仅转发参数的 actorAdmission，直接调用既有调度器，判定字段、阈值和计时边界未改。
原 live 夹具证明同一绑定的三次查询在旧实现额外复制三份原型，改后不再复制。扩展断言
拒绝缺失/非法键，并以不同 argv 生成另一个真实弱键，验证它不能领取原 live 结果，转到
另一键的持久化查询并未命中；原输入变化仍触发 dependency_changed。合并重复查询参数后
原有静态命中、管道拒绝、生产者拒绝、路径集复用和 v6 冷未命中断言保留。完整 Actor 请求
计时仍覆盖首次身份准备，查询计时只少了已删除的重复工作；不据此宣称原生收益已重新测量。
两端 check/build、各 136 项定向及 55 文件单 worker 全量通过：Windows 547 passed / 16 skipped、
WSL 562 passed / 1 skipped；142 个 source/test 文件哈希一致。生产本步 −7 行，累计相对
485cdb2 为 32,637（−197）；测试本步 +7，累计 15,167（+936），无新增用例或文件。
两平台顺序验证，无额外原生成本或压力扫描；ThinkThread、依赖、CI 和 v7 证书格式不变。
完整目标和历史绝对预算仍未完成，macOS/ARM64 与真实 ThinkThread Runtime 未验收。

构造到封存现在共享同一份已归属的规范化原型。createExecPrototype 捕获 argv 一次，直接
进入既有规范化函数；构造时完成校验及环境/FD 排序，stdin、FD 和嵌套普通数据复用既有
cloneSharedData 复制，不冻结调用方数据。模块私有 WeakSet 只标记实际复制、校验、冻结
成功的返回值，后续取键和封存直接复用；外部浅冻结对象与磁盘解析仍须重新捕获校验。
该标记不保存弱键、候选或执行权限，也不增加生命周期所有者。非法身份现在在构造时拒绝，
非字符串平台身份和不透明嵌套值不能进入该已归属集合。原型仍送往实际执行，资源强验证、
生产者策略和 v7 身份算法不变。删除单调用方 actorTopLevelPrototype 包装，Actor 直接使用
同一顶层构造函数；仍先等待平台指纹，再捕获其环境，未改变原调用顺序。
原 ASCII/Unicode 夹具覆盖构造→取键→封存：旧实现各冻结三份原型，新实现只有一份；
三条输入依赖仍只产生三份排除项数组。另复现旧构造器冻结外部 FD，并验证改后调用方可
修改 argv 字节、FD 数组/偏移及浅冻结 stdin 下的嵌套数据，已捕获内容及弱键不漂移。
两份 v7 固定身份、真实 JSON 往返和外部非法输入断言保留；小规模内存对照的 24 组证书
和 384 次非法输入操作无旧/新差异。这些复制次数与对照不证明原生进程耗时或资源资格。
重复的拓扑/产物证书装配合并，原错误与成功断言保留，无新增用例或文件。两端 check/build、
10 文件定向通过（Windows 140 passed / 1 skipped，WSL 141 passed）；55 文件单 worker 全量
通过（Windows 546 passed / 16 skipped，WSL 561 passed / 1 skipped），共 562 项。142 个
source/test 文件哈希一致；生产本步 −6 行，累计相对 485cdb2 为 32,631（−203），测试本步
−21 行，累计 15,146（+915）。平台顺序验证，无额外原生成本或压力扫描；ThinkThread、
依赖、CI 不变。完整目标和历史绝对预算仍未完成，macOS/ARM64 与真实 ThinkThread
Runtime 未验收，既有原生收益记录不冒充本次测量。

原生进程会话的结束现在位于工作区回收之前。旧 world 在 workspaceSandbox.fork 返回或
抛出后才关闭会话，但 fork 的 finally 已删除工作目录；事务驱动只回收自己的句柄，并不
等待 broker 的独立进程或后续证据 IO。现在工具出口结束后先等待 session.close，再进入
工作区最终采集、会话证书封存和回收。顶层执行自身的捕获仍属于顶层执行 Promise，
工作区最终封存仍由原工作区调用等待，不把二者混成 socket 关闭状态。
既有 ActiveSession 在实际调用前登记顶层和完整 broker 处理 Promise；同一关闭 Promise
同步停止新准入，取消顶层/子进程及等待中的交接，等待服务器和已登记任务后才移除 socket。
调用本身的取消信号与会话信号合并，提前调用 session.close 也不能遗留顶层进程；取消后
不会再从候选查找进入顶层回放。移除未使用的 server 字段及空对象 Object.assign 装配，
不增加全局队列、进程管理器、转发层或依赖，继续使用原协调器、spawn、事务和 CAS。
原跨平台真实工作区夹具在旧代码显示：关闭回调进入时目录已经不存在。既有 Linux 并发/
管道拒绝夹具保留原断言，并加入嵌套取消、封存时断线及提前关闭顶层三场景。真实进程组
仅在本夹具中暂停以确定取消边界，不靠忙等或耗时阈值；原生子进程未收到取消、提前关闭
未取消顶层的反例分别复现。封存读取屏障中断开真实 broker socket，并观察原生服务器
关闭回调；仅去掉任务等待的对照仍提前请求移除 socket，恢复后保留 socket 和读取资源，
并发关闭调用共享同一 Promise。夹具 finally 释放读取屏障并等待进程及 trace 清理。
删除该夹具一次重复的后端强制探测，复用现有装配、不新增用例或文件。最终两端 check/build、
10 文件定向通过（Windows 135 passed / 8 skipped，WSL 143 passed），55 文件单 worker 全量
通过（Windows 546 passed / 16 skipped，WSL 561 passed / 1 skipped），共 562 项；142 个
source/test 文件哈希一致。生产本步 +9 行，累计相对 485cdb2 为 32,640（−194），仍不净增；
测试本步 +90 行，累计 15,236（+1,005）。平台顺序验证，无额外原生成本或压力扫描，原收益
记录仍为历史测量。ThinkThread、依赖、CI 和 v7 资格规则不变；完整目标与历史绝对预算
未完成，macOS/ARM64 及真实 ThinkThread Runtime 未验收。

按用户要求暂停新增回归条目，优先审视测试必要性：合并 PatternAware 的训练/反馈、K(a)
投影、sidecar 批次证据和扩展流式采纳夹具；保留正反边界及原元数据断言。删除仅标签不同、
输入和路径相同的平台矩阵、未真正构造晚完成的重复用例，以及只验证三个独立容器互不影响
的伪集成测试。未启动请求不再重复晚 resolve/reject，准备语义不在关闭夹具中再次执行。
11 个测试文件净减 493 行，15,236 → 14,743；条目 562 → 545，未增加文件或跳过项。
后续回归优先重构既有测试，禁止无独立证明价值地新增条目。两端 check 和定向通过
（Windows 226 passed / 1 skipped，WSL 227 passed）；55 文件单 worker 全量通过
（Windows 529 passed / 16 skipped，WSL 544 passed / 1 skipped）。平台顺序运行，未追加
