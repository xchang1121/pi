...
impl Job {
    /// Lifecycle-only job for the internal AppContainer broker.
    pub fn supervisor() -> Result<Self> {
        Self::create(true, false)
    }

    /// Load-bearing containment job for the final sandbox command.
    pub fn sandbox() -> Result<Self> {
        Self::create(false, true)
    }

    /// Create an unnamed job with kill-on-close + full UI lockdown.
    /// The outer broker job permits one breakaway so the final child can be
    /// assigned to the stricter sandbox job. The sandbox job never permits
    /// child breakaway.
    fn create(breakaway_ok: bool, restrict_ui: bool) -> Result<Self> {
...
/// one breaks assignment to the final sandbox job.
