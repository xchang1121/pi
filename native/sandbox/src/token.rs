...
//! Restricted base token for a zero-capability AppContainer process.
//!
//! Administrators and interactive logon SIDs are deny-only, all privileges
//! except `SeChangeNotifyPrivilege` are removed, and the token is marked LUA.
//! Windows then derives the final low-integrity AppContainer token from this
//! base token through `PROC_THREAD_ATTRIBUTE_SECURITY_CAPABILITIES`.
...
use windows::Win32::Security::{
    ...
    TokenDefaultDacl, TokenGroups, TokenIntegrityLevel, TokenIsAppContainer, TokenPrimary,
...
/// Medium IL for the restricted base token. AppContainer creation lowers the
/// final process token to AppContainer low integrity.
...
/// Give SYSTEM and the base user access to objects created by descendants.
