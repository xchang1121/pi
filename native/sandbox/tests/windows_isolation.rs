...
fn timeout_kills_the_process_tree() {
    ...
    let (_root, source, sandbox) = workspace();
    fs::write(
        sandbox.join("late.ps1"),
        "Start-Sleep -Seconds 2; [IO.File]::WriteAllText((Join-Path $PWD 'late.txt'), 'leaked')",
    )
    .unwrap();
    fs::write(
        sandbox.join("parent.ps1"),
        "$child = Start-Process powershell.exe -ArgumentList '-NoProfile','-NonInteractive','-File',\"$PWD\\late.ps1\" -PassThru; Start-Sleep -Seconds 20",
    )
    .unwrap();
    let response = run(
        &source,
        &sandbox,
        "powershell.exe -NoProfile -NonInteractive -File parent.ps1",
        500,
        16 * 1024,
    );
